...
const MainPage = () => {
  ...
  const { goalSeriesLimit, yCardsSeriesLimit, setGoalSeriesLimit, setYCardsSeriesLimit } = useSeriesLimits();

  const minTimeStamp = 1687282001;
  const maxTimeStamp = 1687292001;

  const handleSeriesLimitChange = (id, limit) => {
    (id === 'goals' && setGoalSeriesLimit(limit)) || (id === 'yCards' && setYCardsSeriesLimit(limit))
  }
          ...
          {items.map((item) => (
            item.isLoading || mainInfo.isLoading ? (
              ...
              <SortableItemSeries
                ...
                seriesLimitChange={handleSeriesLimitChange}
              />
            )
          ))}
        </SortableContext>
